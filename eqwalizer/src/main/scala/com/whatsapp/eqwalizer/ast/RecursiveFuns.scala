...
class RecursiveFuns(module: String) {
  ...
  var foon2Based = Map[FoonType, Option[FoonType]]()

  /**
   * Note: This rewriting will not be necessary when we have type checker
   * error recovery.
   *
   *
   * Rewrite the abstract forms (if possible) so that only non-self-recursive
   * branches remain.
   *
  ...
  def baseFoon(f: FoonType): Option[FoonType] =
    foon2Based.get(f).getOrElse({
      val based = baseClauses(f.clauses, FoonType(_, f.module, f.env))
      foon2Based += f -> based
      based
    })
  ...
  private def base(e: Expr): Option[Expr] = e match {
    case Block(exprs0) =>
    ...
    case LocalCall(id, args) =>
      Util.getFunType(module, id) match {
        case Some(f: FoonType) => baseFoon(f)
        case None =>
      }
      if (id == funId) None
      else mapBase(args) map (LocalCall(id, _)(e.l))
    case RemoteCall(id, args) =>
      if (id.name == funId.name && id.arity == funId.arity) None
