...
import erlang.forms.AbstractGuard._

object AbstractGuardConvert {
  def convertGuard(term: EObject): Guard =
    term match {
      case EList(tests, None) =>
        Guard(tests.map(convertTest))
    }

  def convertTest(term: EObject): Test =
    term match {
      case ETuple(List(EAtom("var"), _anno, EAtom(name))) =>
        TestVariable(name)
      case ETuple(List(EAtom("tuple"), _anno, EList(eTests, None))) =>
      ...
      case ETuple(List(EAtom("record"), _anno, EAtom(recordName), EList(eRecordFieldTests, None))) =>
      ...
      case ETuple(List(EAtom("map"), _anno, EList(eAssocs, None))) =>
        TestMapCreation(eAssocs.map(AbstractExprConvert.convertAssoc))
      case ETuple(List(EAtom("map"), _anno, eExp, EList(eAssocs, None))) =>
        TestMapUpdate(AbstractExprConvert.convertExp(eExp), eAssocs.map(AbstractExprConvert.convertAssoc))
      case ETuple(
            List(EAtom("call"), _anno, ETuple(List(EAtom("remote"), _anno1, erlang, eFun)), EList(eTests, None))
          ) =>
        val Some(AF_LiteralAtom("erlang")) = AbstractExprConvert.maybeLiteral(erlang)
        val Some(AF_LiteralAtom(funName)) = AbstractExprConvert.maybeLiteral(eFun)
        TestGuardErlangCall(funName, eTests.map(convertTest))
      case ETuple(List(EAtom("call"), _anno, eFun, EList(eTests, None))) =>
        val Some(AF_LiteralAtom(funName)) = AbstractExprConvert.maybeLiteral(eFun)
        TestGuardCall(funName, eTests.map(convertTest))
      case _ =>
  ...
  def convertPatternBinElement(term: EObject): TestBinElement =
    term match {
      case ETuple(List(EAtom("bin_element"), _anno, eTest, eSize, eTypeSpecifiers)) =>
        TestBinElement(convertTest(eTest), eSize, AbstractExprConvert.convertTypeSpecifiers(eTypeSpecifiers))
    }

  def convertRecordFieldPattern(term: EObject): RecordFieldTest =
    term match {
