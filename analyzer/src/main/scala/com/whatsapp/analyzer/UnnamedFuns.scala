...
object UnnamedFuns {
  type Line = Int
  case class Info(module: String, unnamedFuns: Int)

  def main(args: Array[String]): Unit = {
    val infos = Using.resource(RPC.connect())(loadData)
    var unnamedFunsCount: Int = 0
    for (info <- infos) {
      unnamedFunsCount += info.unnamedFuns
    }
    println(s"Total: $unnamedFunsCount unnamed funs")
  }
  ...
  private def indexProjectDir(rpc: RPC)(dir: String): List[Info] = {
    ...
    moduleNames map { m => Info(m, rpc.getUnnamedFuns(s"$dir/${m}.beam")) }
  }
}
