...
object ParameterizedTypes {
  case class Info(module: String, parameterizedTypes: List[(Int, String)])

  def main(args: Array[String]): Unit = {
    val infos = Using.resource(RPC.connect())(loadData)
    var count: Int = 0
    var uniqueNames: Set[String] = Set.empty
    for (info <- infos) {
      if (info.parameterizedTypes.nonEmpty) {
        count += info.parameterizedTypes.length
        uniqueNames ++= info.parameterizedTypes.map(_._2)
        println(s"${info.module}: ${info.parameterizedTypes.mkString(", ")}")
      }
    }
    println(s"Total: $count parameterized types")
    println(s"Unique: ${uniqueNames.mkString(", ")}")
    println(s"Unique size: ${uniqueNames.size}")
  }
  ...
  private def indexProjectDir(rpc: RPC)(dir: String): List[Info] = {
    ...
    moduleNames map { m => Info(m, rpc.getParameterizedTypes(s"$dir/${m}.beam")) }
  }
}
