...
object NamedFuns {
  type Line = Int
  case class Info(module: String, namedFuns: List[(Int, String)])

  def main(args: Array[String]): Unit = {
    val infos = Using.resource(RPC.connect())(loadData)
    var namedFunsCount: Int = 0
    for (info <- infos) {
      if (info.namedFuns.nonEmpty) {
        namedFunsCount += info.namedFuns.length
        println(s"${info.module}: ${info.namedFuns.mkString(", ")}")
      }
    }
    println(s"Total: $namedFunsCount named funs")
  }
  ...
  private def indexProjectDir(rpc: RPC)(dir: String): List[Info] = {
    ...
    moduleNames map { m => Info(m, rpc.getNamedFuns(s"$dir/${m}.beam")) }
  }
}
