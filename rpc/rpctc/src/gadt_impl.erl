...
gen(Service) ->
    ...
    "-spec handle_call(call_request(), pid(), {impl_module(Input, State), State}) -> {reply, term(), {impl_module(Input, State), State}}.\n"
    "handle_call(Request, _From, {ImplModule, State}) ->\n"
    "    case Request of\n"
    "" ++ lists:join(";\n", lists:map(fun handle_call_str/1, Calls)) ++ "\n"
    "    end.\n"
    "\n"
    "-spec handle_cast(cast_request(), {impl_module(Input, State), State}) -> {noreply, {impl_module(Input, State), State}}.\n"
...
cast_str(#cast{name = Name, params = Params}) ->
    ...
    "" ++ Name ++ "(Name, " ++ lists:join(", ", args(0, Params)) ++ ") ->\n"
    "    gen_server:cast(Name, cast_request." ++ Name ++ "{" ++ lists:join(", ", args(0, Params)) ++ "}).".
