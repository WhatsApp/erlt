...
gen_call(#cast{name = Name, params = Params}) ->
    ...
    "    gen_server:cast(?MODULE, cast_request." ++ Name ++ "{" ++ lists:join(", ", args(0, Params)) ++ "}).\n"
    ;
...
handle_cast_spec(#cast{name = Name, params = Params}) ->
    "    (cast_request." ++ Name ++ "{" ++ lists:join(", ", Params) ++ "}, state()) -> {noreply, state()}".

%% erlfmt-ignore
handle_call_spec(#call{name = Name, params = Params, return = Return}) ->
    "    (call_request." ++ Name ++ "{" ++ lists:join(", ", Params) ++ "}, pid(), state()) -> {reply, " ++ Return ++ ", state()}".
