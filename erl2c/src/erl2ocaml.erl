...
do_erl2ocaml_ffi(Forms) ->
    ...
    {PubSpecs, SpecIds} = lists:unzip(
        [
            {PubSpec, FId}
            || PubSpec = {_, _, _, {FId, _}} <- AllSpecs, lists:member(FId, Exports)
        ]
    ),
    ...
    case UnSpecedFunIds of
        ...
        _ ->
            [{L, N, A} | _] =
                [
                    {L, N, A}
                    || {_, L, N, A, _} <- Funs, lists:member({N, A}, UnSpecedFunIds)
                ],
    ...
    ExtraGuard =
        case Env1 of
            ...
            _ ->
                [Eq1 | Eqs] = lists:map(
                    fun ({K, V}) ->
                        {op, Line, '==', {'var', Line, K}, {'var', Line, V}}
                    end,
                    Env1
                ),
                lists:foldl(fun (H, Acc) -> {op, Line, 'and', Acc, H} end, Eq1, Eqs)
        end,
    ExtraBody = lists:map(fun ({P, E}) -> {match_r, Line, P, E} end, Env2),
...
clause1({clause, Line, Head, Guard, Body}, ExtraBody, ExtraGuard, Ctx) ->
    CombinedGuard =
        case Guard of
            ...
            _ ->
                ...
                Guard2 = lists:foldl(
                    fun (H, Acc) -> {op, Line, 'or', Acc, H} end,
                    Guard1,
                    Guards1
                ),
                case ExtraGuard of
    ...
    case {ExtraBody, CombinedGuard} of
        ...
        {_, _} ->
            ...
            [
                {"| " ++ head(Head, Ctx) ++ " ->"},
                expr_seq(ExtraBody ++ CombinedGuardInBody ++ Body, Ctx)
            ]
    end.
...
expr1(Acc, [{match, Ln, P, E} | Es], Ctx) ->
    ...
    RMs1 = lists:map(
        fun ({K, V}) ->
            {match_r, Ln, {atom, Ln, true}, {op, Ln, '==', {'var', Ln, K}, {'var', Ln, V}}}
        end,
        Env1
    ),
    RMs2 = lists:map(fun ({Ptr, Exp}) -> {match_r, Ln, Ptr, Exp} end, Env2),
...
expr({map, Line, Map, UpdateAssocs}, Ctx) ->
    SameFields =
        [
            {op, Line, '==', V, {'op', Line, '.', Map, K}}
            || {map_field_exact, _, K = {atom, _, _}, V} <- UpdateAssocs
        ],
    SameMaps =
...
expr({'case', _Line, Expr, Clauses}, Ctx) ->
    [{"("}, {"match"}] ++
        [expr(Expr, Ctx)] ++ [{"with"}] ++ [clauses(Clauses, Ctx)] ++ [{")"}];
...
expr({named_fun, Line, Name, Clauses}, Ctx) ->
    Exprs = [
        {match_rec, Line, {var, Line, Name}, {'fun', Line, {clauses, Clauses}}},
        {var, Line, Name}
    ],
    expr({block, Line, Exprs}, Ctx);
expr({call, L1, {atom, _, F}, As}, Ctx) ->
...
init_assoc({map_field_assoc, Line, E, _V}, _Ctx) ->
    throw(
        {erl2ocaml_error,
            {Line, {'not_supported_syntax', E, "non-atom key in map_field_assoc"}}}
    ).
...
update_assoc(_Acc, {map_field_exact, Line, E, _V}, _Ctx) ->
    throw(
        {erl2ocaml_error,
            {Line, {'not_supported_syntax', E, "non-atom key in map_field_exact"}}}
    );
...
enum_alias(N, TVs = [TV], Ctx) ->
    type(TV, Ctx) ++
        " " ++ type_name(N, TVs) ++ " = " ++ type(TV, Ctx) ++ " " ++ atom_to_list(N);
...
erl2ocaml_spec({attribute, Line, spec, Spec}, _) ->
    throw(
        {erl2ocaml_error,
            {Line, {'not_supported_syntax', Spec, "spec in not supported format"}}}
    ).
...
map_field_type({type, _, map_field_exact, [{atom, _, K}, VT]}, SelfType, Ctx) ->
    ...
    "get_" ++
        atom_to_list(K) ++
        " : " ++
        VType ++ "; set_" ++ atom_to_list(K) ++ " : (" ++ VType ++ ") ->" ++ SelfType;
...
mk_sccs(Functions) ->
    ...
    FunsIndex =
        lists:map(fun (Fun) -> {get_fn_name(Fun), get_fn_arg_len(Fun)} end, Functions),
    OFuns = lists:foldl(fun (F, Map) -> Map#{F => maps:size(Map)} end, #{}, FunsIndex),
    SCCs1 = lists:map(
        fun (SCC) ->
            lists:sort(fun (A, B) -> maps:get(A, OFuns) < maps:get(B, OFuns) end, SCC)
        end,
        SCCs
    ),
    SCCs2 = lists:sort(
        fun ([F1 | _], [F2 | _]) -> maps:get(F1, OFuns) < maps:get(F2, OFuns) end,
        SCCs1
    ),
        ...
        fun (FromSCC) ->
            ...
            lists:map(
                fun (FunV) ->
                    ...
                    ToSCCs =
                        lists:map(fun (Nb) -> find_parent_sCC(SCCs, Nb) end, Neighbours),
                    lists:map(
                        fun (ToSCC) ->
...
find_parent_sCC(SCCs, FunV) ->
    Parents = lists:dropwhile(
        fun (SCC) -> not lists:member(FunV, SCC) end,
        SCCs
    ),
    lists:nth(1, Parents).
