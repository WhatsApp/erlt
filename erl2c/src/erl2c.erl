...
usage() ->
    H = [{"-b type","type of output file (e.g. beam)"},
	 ...
	 {"-M2C","(temporary) -M2 with -M compatibility mode: -M2C means -M2 wihtout .beam dependencies; -M2C output should be identical to -M"},
...
compile3(File, Cwd, Options) ->
    ...
    case filename:extension(File) of
		...
		{'EXIT',Reason} ->
		    io:format(?STDERR,
			      "erl2 compiler failed:\n~p~n",
			      [Reason]),
		    error;
		Other ->
		    io:format(?STDERR,
			      "erl2 compiler returned:\n~p~n",
			      [Other]),
		    error
	    end;
        _ ->
