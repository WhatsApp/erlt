...
keep_error({undefined_enum_constructor, _E, _A, _N}, _Def) ->
    true;
keep_error({enum_constructor_wrong_arity, _E, _A, _N}, _Def) ->
    true;
keep_error({redefine_import_type, {{_F, _A}, _M}}, _Def) ->
    true;
keep_error({unused_import_type, {{_F, _A}, _M}}, _Def) ->
    true;
...
keep_error({undefined_type, {_T, _A}}, _Def) ->
    true;
keep_error({shadowed_var, _V, _In}, _Def) ->
    true;
keep_error({exported_var, _V, {_What, _Where}}, _Def) ->
    true;
...
start(File, Opts) ->
    Enabled0 =
        [
            ...
            {shadow_vars, true},
            {unused_import,
                bool_option(warn_unused_import, nowarn_unused_import, false, Opts)},
            {unused_function,
                bool_option(warn_unused_function, nowarn_unused_function, true, Opts)},
            ...
            {unused_record,
                bool_option(warn_unused_record, nowarn_unused_record, true, Opts)},
            {deprecated_function,
                bool_option(
            ...
            {deprecated_type,
                bool_option(warn_deprecated_type, nowarn_deprecated_type, true, Opts)},
            {obsolete_guard,
                bool_option(warn_obsolete_guard, nowarn_obsolete_guard, true, Opts)},
            {untyped_record,
                bool_option(warn_untyped_record, nowarn_untyped_record, false, Opts)},
            {missing_spec,
                bool_option(warn_missing_spec, nowarn_missing_spec, false, Opts)},
            {missing_spec_all,
...
not_deprecated(Forms, #lint{compile = Opts} = St0) ->
    ...
    Nowarn = [
        MFA
        || {nowarn_deprecated_function, MFAs0} <- Opts, MFA <- lists:flatten([MFAs0])
    ],
            ...
            ({map_field_exact, L, K, V}, {Psvt, Bvt0, St0}) ->
                case is_valid_map_key(K) of
                    true ->
                        ...
                        {vtmerge_pat(vtmerge_pat(Kvt, Vvt), Psvt), vtmerge_pat(Bvt0, Bvt2),
                            St2};
                    false ->
...
expr({'try', Line, Es, Scs, Ccs, As}, Vt, St0) ->
    ...
    {Sccs, St2} = try_clauses(Scs, Ccs, TryLine, vtupdate(Evt1, Vt), St1),
    Rvt0 = Sccs,
...
check_enum({atom, L, E} = E0, {atom, _, A} = A0, Args, St) ->
    case imported_type(E, St) of
        ...
        no ->
            ...
            N = length(Args),
            case maps:find(E, St#lint.enums) of
                error ->
                    add_error(L, {undefined_enum, E}, St);
                {ok, Cs} ->
                    case lists:keyfind(A, 1, Cs) of
                        {_, N} ->
                            St;
                        {_, N1} ->
                            add_error(L, {enum_constructor_wrong_arity, E, A, N1}, St);
                        false ->
                            add_error(L, {undefined_enum_constructor, E, A, N}, St)
                    end
            end
    end;
    ...
    Warns =
        case Type of
            all ->
                [
                    {FA, L}
                    || {function, L, F, A, _} <- Forms, not lists:member(FA = {F, A}, Specs)
                ];
            exported ->
