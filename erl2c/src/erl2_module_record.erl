...
desugar_module_record_attribute([
    {attribute,Line,record,{{module_record, _Line3, Module, ModuleRecordName}, Fields}} | T]) ->
    RecordTag = list_to_atom(lists:append([atom_to_list(Module), ":", atom_to_list(ModuleRecordName)])),
...
do_parse_transform(Forms, ModuleRecords, ModuleRecordUsages) ->
    ...
    Forms2 = lists:flatmap(fun (Form) -> inject_module_record_forms(Form, RecordForms) end, Forms1),
    %% For each remote module which module records are used,
    %% inject all -record(record_tag, ...) forms for module records.
    Forms3 = lists:flatmap(fun (Form) -> inject_remote_module_record_forms(Form, Context) end, Forms2),
    %% Done.
    Forms3.
...
form({attribute,Line,file,{File,Line}},_Context) ->	%This is valid anywhere.
...
form({attribute,Line,Attr,Val},_Context) ->		%The general attribute.
...
form({error,E},_Context) -> {error,E};
form({warning,W},_Context) -> {warning,W};
form({eof,Line},_Context) -> {eof,Line}.
...
record_defs([{typed_record_field,{record_field,Line,{atom,La,A},Val0},Type}|Is],Context) ->
    Val1 = expr(Val0,Context),
    Type1 = type(Type,Context),
    [{typed_record_field,{record_field,Line,{atom,La,A},Val1},Type1}|record_defs(Is,Context)];
record_defs([{typed_record_field,{record_field,Line,{atom,La,A}},Type}|Is],Context) ->
    Type1 = type(Type,Context),
    [{typed_record_field,{record_field,Line,{atom,La,A}},Type1}|record_defs(Is,Context)];
...
pattern({var,Line,V},_Context) -> {var,Line,V};
pattern({match,Line,L0,R0},Context) ->
    L1 = pattern(L0,Context),
    R1 = pattern(R0,Context),
    {match,Line,L1,R1};
pattern({integer,Line,I},_Context) -> {integer,Line,I};
...
gexpr({var,Line,V},_Context) -> {var,Line,V};
gexpr({integer,Line,I},_Context) -> {integer,Line,I};
...
gexpr({cons,Line,H0,T0},Context) ->
    H1 = gexpr(H0,Context),
    T1 = gexpr(T0,Context),				%They see the same variables
    {cons,Line,H1,T1};
...
gexpr({call,Line,{atom,La,F},As0},Context) ->
    case erl_internal:guard_bif(F, length(As0)) of
        true -> As1 = gexpr_list(As0,Context),
            {call,Line,{atom,La,F},As1}
    end;
...
expr({var,Line,V},_Context) -> {var,Line,V};
...
expr({cons,Line,H0,T0},Context) ->
    H1 = expr(H0,Context),
    T1 = expr(T0,Context),				%They see the same variables
    {cons,Line,H1,T1};
...
type({type,Line,record,[QR={qualified_record,{atom,_,_},{atom,_,_}}|Fs]},Context) ->
    Fs1 = field_types(Fs,Context),
...
collect_record_usage({qualified_record,Module,Record}) when is_atom(Module), is_atom(Record) ->
    {Module, Record};
collect_record_usage({qualified_record,{atom,_,Module},{atom,_,Record}}) when is_atom(Module), is_atom(Record) ->
    {Module, Record};
...
init_module_record_context(Forms, ModuleRecords, ModuleRecordUsages) ->
    ...
    RemoteMapping = [{M, maps:from_list(RemoteModuleRecords)} || {M,{RemoteModuleRecords,_}} <- RemoteInfo],
...
resolve_record({qualified_record,M,MR},_Line,Context) when is_atom(M), is_atom(MR) ->
    ...
    resolve_qualified_record(M,MR,ModuleRecordTags);
%% Used from types
resolve_record({qualified_record,{atom,Line,M},{atom,_,MR}},_Line,Context) when is_atom(M), is_atom(MR) ->
    ModuleRecordTags = Context#module_record_context.module_record_tags,
...
inject_module_record_forms(ModuleRecord={attribute,Line,module_record,{_,RecordTag}}, RecordForms) ->
