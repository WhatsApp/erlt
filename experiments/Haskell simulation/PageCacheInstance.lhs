...
> module PageCacheInstance(State,PageCache,start,get,getAll,put) where

> import Prelude hiding (init)
> import qualified Data.Map.Strict as Map
> import Processes
> import GenServerClass hiding (start)
> import qualified GenServerClass as GenServer

We have to implement a generic server behaviour. In this variant we do
it just by defining in instance of the Behaviour class.
...
We define type aliases for page contents and the page server state.
...
> start initialState = PageCache <$> GenServer.start (State initialState)
