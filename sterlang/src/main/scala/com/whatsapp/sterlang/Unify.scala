...
class Unify(val vars: Vars) {
  ...
  private def adjust(v: UVar, d: T.Depth, t: T.Type): Unit = {
    ...
    def loop(v: UVar)(t: T.Type): Unit = t match {
      case T.VarType(tv1) =>
        if (!visited(tv1)) {
          visited = visited + tv1
          vars.tGet(tv1) match {
    ...
    def rowLoop(v: UVar)(rt: T.RowType): Unit = rt match {
  ...
  private def unifyV(tv: T.TypeVar, conType: T.ConType): Unit = vars.tGet(tv) match {
    case T.Instance(t) =>
      unify(t, conType)
    case T.Open(d) =>
  ...
  private def rowUnify(rt1: T.RowType, rt2: T.RowType): Unit = {
    @scala.annotation.tailrec
    def collect(rowType: T.RowType,
                m: Map[String, T.Field]): (Map[String, T.Field], Option[(T.RowTypeVar, T.RowOpen)]) =
      rowType match {
        case T.RowVarType(v) => vars.rGet(v) match {
  ...
  @throws[UnifyError]
  def unify(t1: T.Type, t2: T.Type): Unit = (t1, t2) match {
