...
object Parser extends StandardTokenParsers with PackratParsers with ImplicitConversions {
  ...
  private def pos[T](p: => Parser[Pos.P => T]): Parser[T] = Parser { in1 =>
    p(in1) match {
  ...
  lazy val varName: PackratParser[S.VarName] =
    (lident <~ ":") ~ lident ~ (guard(args) ^^ {_.size}) ^^ {case id1 ~ id2 ~ a => new S.RemoteFunName(id1, id2, a)} |
  ...
  lazy val specTp: PackratParser[S.Spec] =
    pos(lident ~ typeArgs ~("->" ~> tp) ^^ {
      case name ~ params ~ res => (p: Pos.P) =>
        S.Spec(new S.LocalFunName(name, params.size), S.FunType(params, res)(p))(p)
    })
