...
class TypePrinter(private val vars: Vars, private val tu: TypesUtil) {
  ...
  private case class Info(sTypeVars: Set[ST.TypeVar] = Set.empty,
  ...
  private def collectInfo(body: ST.Type): (ST.Type, Info) = {
    ...
    def typ(ts: ST.Type): (ST.Type, Info) = ts match {
    ...
    def rtyp(rts: ST.RowType): (ST.RowType, Info) = rts match {
  ...
  def typeSchema(schema: ST.TypeSchema, mode: TypePrinter2.Mode): String = {
    ...
    val sRowTypeVarNames: IntMap[String] =
      schema.rargs.indices.foldLeft(IntMap.empty[String]){case (m, i) => m + (i -> localNames.nextSchematicRowTypeName())}
    ...
    def typ(sType: ST.Type): String = sType match {
    ...
    def conType(tyCon: TC.TyCon,
                sTypes: List[ST.Type],
                sRowTypes: List[ST.RowType]): String = (tyCon, sTypes, sRowTypes) match {
    ...
    def recordStr(sRowType: ST.RowType): String = sRowType match {
    ...
    def rowtype(sRowType: ST.RowType,
                lb: String,
                rb: String): String = {
      ...
      def unfold(sRowType: ST.RowType, fields: List[ST.Field]): String = sRowType match {
        case ST.RowEmptyType =>
