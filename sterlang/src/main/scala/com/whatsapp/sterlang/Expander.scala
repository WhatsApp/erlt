...
class Expander(val aliases: List[Ast.TypeAlias],
               val tGen: () => Types.Type,
  ...
  def mkType(t: S.Type, sub: Map[String, T.Type]): T.Type = t match {
    case S.TypeVar(n) =>
      sub(n)
    case S.WildTypeVar() =>
      tGen()
  ...
  def mkSType(t: S.Type, sub: Map[String, ST.Type]): ST.Type = t match {
    case S.TypeVar(n) =>
      sub(n)
    case S.WildTypeVar() =>
      ST.PlainType(tGen())
  ...
  def expandSType(ts: ST.Type): ST.Type = ts match {
  ...
  private def expandRSType(rts: ST.RowType): ST.RowType = rts match {
  ...
  def expandType(t: T.Type): T.Type = t match {
    case T.VarType(tv) =>
      T.VarType(tv)
  ...
  private def expandRowType(rt: T.RowType): T.RowType = rt match {
