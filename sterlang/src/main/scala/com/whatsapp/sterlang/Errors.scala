...
class UnknownInterface(range: Doc.Range, val intName: String)  extends RangeError(range, s"Unknown interface: $intName", None)
class BadInterface(range: Doc.Range, val intName: String)  extends RangeError(range, s"Interface $intName is not a shape", None)
