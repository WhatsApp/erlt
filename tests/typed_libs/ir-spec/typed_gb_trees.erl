...
-opaque tree(Key, Value) :: {non_neg_integer(),
                             gb_tree_node(Key, Value)}.

-opaque iter(Key, Value) :: [gb_tree_node(Key, Value)].

-type gb_tree_node(K,
                   V) :: {'$#typed_gb_trees:gb_tree_node.nil'} |
                         {'$#typed_gb_trees:gb_tree_node.branch',
                          K,
                          V,
                          gb_tree_node(K, V),
                          gb_tree_node(K, V)}.
...
empty() -> {0, {'$#typed_gb_trees:gb_tree_node.nil'}}.

-spec is_empty(tree(_K, _V)) -> boolean().

is_empty({0, {'$#typed_gb_trees:gb_tree_node.nil'}}) ->
    true;
is_empty(_) -> false.
...
lookup_1(_, {'$#typed_gb_trees:gb_tree_node.nil'}) ->
    {'$#types:option.none'}.
...
is_defined_1(_,
             {'$#typed_gb_trees:gb_tree_node.branch', _, _, _, _}) ->
    true;
is_defined_1(_,
             {'$#typed_gb_trees:gb_tree_node.nil'}) ->
    false.
...
-type insert_return(Key,
                    Value) :: {'$#typed_gb_trees:insert_return.plain',
                               gb_tree_node(Key, Value)} |
...
insert_1(Key, Value,
    ...
    case insert_1(Key, Value, Smaller, S bsr 1) of
        {'$#typed_gb_trees:insert_return.rebalance',
         ...
         H1,
         S1} ->
            T = {'$#typed_gb_trees:gb_tree_node.branch',
                 Key1,
                 V,
                 T1,
                 Bigger},
...
insert_1(Key, Value,
         {'$#typed_gb_trees:gb_tree_node.nil'}, S)
    when S =:= 0 ->
    {'$#typed_gb_trees:insert_return.rebalance',
     {'$#typed_gb_trees:gb_tree_node.branch',
      Key,
     ...
     1};
insert_1(Key, Value,
         {'$#typed_gb_trees:gb_tree_node.nil'}, _S) ->
    {'$#typed_gb_trees:insert_return.plain',
     {'$#typed_gb_trees:gb_tree_node.branch',
      Key,
      Value,
...
count({'$#typed_gb_trees:gb_tree_node.branch',
       _,
       _,
       {'$#typed_gb_trees:gb_tree_node.nil'},
       {'$#typed_gb_trees:gb_tree_node.nil'}}) ->
...
count({'$#typed_gb_trees:gb_tree_node.nil'}) -> {1, 0}.
...
balance_list_1([{Key, Val} | L], 1) ->
    {{'$#typed_gb_trees:gb_tree_node.branch',
     ...
     L};
balance_list_1(L, 0) ->
    {{'$#typed_gb_trees:gb_tree_node.nil'}, L}.
...
merge(Smaller, {'$#typed_gb_trees:gb_tree_node.nil'}) ->
    Smaller;
merge({'$#typed_gb_trees:gb_tree_node.nil'}, Larger) ->
    Larger;
merge(Smaller, Larger) ->
...
take_smallest1({'$#typed_gb_trees:gb_tree_node.branch',
                Key,
                Value,
                {'$#typed_gb_trees:gb_tree_node.nil'},
                Larger}) ->
...
smallest_1({'$#typed_gb_trees:gb_tree_node.branch',
            Key,
            Value,
            {'$#typed_gb_trees:gb_tree_node.nil'},
            _Larger}) ->
...
take_largest1({'$#typed_gb_trees:gb_tree_node.branch',
               Key,
               Value,
               Smaller,
               {'$#typed_gb_trees:gb_tree_node.nil'}}) ->
...
largest_1({'$#typed_gb_trees:gb_tree_node.branch',
           Key,
           Value,
           _Smaller,
           {'$#typed_gb_trees:gb_tree_node.nil'}}) ->
...
to_list({'$#typed_gb_trees:gb_tree_node.branch',
         Key,
         Value,
         Small,
         Big},
...
to_list({'$#typed_gb_trees:gb_tree_node.nil'}, L) -> L.

-spec keys(tree(Key, _Value)) -> [Key].

keys({_, T}) -> keys(T, []).

keys({'$#typed_gb_trees:gb_tree_node.branch',
      Key,
      _Value,
      Small,
      Big},
...
keys({'$#typed_gb_trees:gb_tree_node.nil'}, L) -> L.

-spec values(tree(_Key, Value)) -> [Value].

values({_, T}) -> values(T, []).

values({'$#typed_gb_trees:gb_tree_node.branch',
        _Key,
        Value,
        Small,
        Big},
...
values({'$#typed_gb_trees:gb_tree_node.nil'}, L) -> L.
...
iterator({'$#typed_gb_trees:gb_tree_node.branch',
          _,
          _,
          {'$#typed_gb_trees:gb_tree_node.nil'},
          _} =
...
iterator({'$#typed_gb_trees:gb_tree_node.branch',
         ...
         As) ->
    iterator(L, [T | As]);
iterator({'$#typed_gb_trees:gb_tree_node.nil'}, As) ->
    As.
...
iterator_from(S,
              {'$#typed_gb_trees:gb_tree_node.branch', K, _, _, T},
              As)
    when K < S ->
...
iterator_from(_,
    ...
    [T | As];
iterator_from(S,
              {'$#typed_gb_trees:gb_tree_node.branch', _, _, L, _} =
                  T,
              As) ->
    iterator_from(S, L, [T | As]);
iterator_from(_, {'$#typed_gb_trees:gb_tree_node.nil'},
              As) ->
    As.
...
next([{'$#typed_gb_trees:gb_tree_node.branch',
       X,
       V,
       _,
       T}
...
map_1(_, {'$#typed_gb_trees:gb_tree_node.nil'}) ->
    {'$#typed_gb_trees:gb_tree_node.nil'};
map_1(F,
      {'$#typed_gb_trees:gb_tree_node.branch',
       K,
