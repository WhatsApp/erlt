...
let test =
      ...
      , steps =
        ...
        , run "test stErlang" "cd sterlang && sbt 'coverage;test'"
        , run "test erltc + stErlang in dev mode" "make -C sterlang/examples build-jar"
        , run "coverageReport" "cd sterlang && sbt 'coverageReport'"
        ]
      }
      ...
      λ(runsOn : Actions.RunsOn.Type) →
        ...
        , steps =
            ...
            # [run
                  ...
                  "mkdir -p erltc/priv && cp ${nativeName} erltc/priv/${nativeName} && make -C tests test-native"
            ]
        }
