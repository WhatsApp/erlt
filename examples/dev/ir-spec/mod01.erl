...
-spec update_shape2(#{a := A, atom() => any()}, A) ->
                       #{a := A, atom() => any()}.
...
-spec update_shape3(#{a := A, b := A}) -> #{a := A, b := A}.
...
-spec update_shape4(#{a := A, b := A, atom() => any()}) ->
                       #{a := A, b := A, atom() => any()}.
...
-spec access_shape(#{id := Id, location := Location, atom() => any()}) ->
                      {Id, Location}.


access_shape(S) ->
    {map_get(id, S), map_get(location, S)}.
...
access_shape2(S) ->
    map_get(inner2, map_get(inner1, S)).
