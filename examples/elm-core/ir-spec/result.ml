...
let rec map_error'2 : ('tX -> 'tY) * ('tX, 'tA) result'2 -> ('tY, 'tA) result'2
    = function
  | v__F, Ok v_V -> Ok v_V
  | v_F, Err v_E -> Err (v_F v_E)

let rec map_error'1 : ('tX -> 'tY) -> ('tX, 'tA) result'2 -> ('tY, 'tA) result'2
    = function
...
let rec from_maybe'2 : 'tX * 'tA Maybe.maybe'1 -> ('tX, 'tA) result'2 = function
