...
let rec empty'0 : unit -> ('t_K, 't_V) map = function () -> Map_ffi.empty'0 ()

let rec get'2 : 'tK * ('tK, 'tV) map -> 'tV Maybe.maybe = function
  | v_Key, v_Map -> Map_ffi.get'2 (v_Key, v_Map)

let rec member'2 : 'tK * ('tK, 't_V) map -> bool = function
  | v_Key, v_Map -> (
      match get'2 (v_Key, v_Map) with
      | Maybe.Just _ -> true
      | Maybe.Nothing -> false )

let rec size'1 : ('t_K, 't_V) map -> int = function
  | v_Map -> Map_ffi.size'1 v_Map

let rec is_empty'1 : ('t_K, 't_V) map -> bool = function
  | v_Map -> size'1 v_Map = 0

let rec insert'3 : 'tK * 'tV * ('tK, 'tV) map -> ('tK, 'tV) map = function
  | v_Key, v_Value, v_Map -> Map_ffi.insert'3 (v_Key, v_Value, v_Map)

let rec remove'2 : 'tK * ('tK, 'tV) map -> ('tK, 'tV) map = function
  | v_Key, v_Map -> Map_ffi.remove'2 (v_Key, v_Map)

let rec update'3 :
    'tK * ('tV Maybe.maybe -> 'tV Maybe.maybe) * ('tK, 'tV) map ->
    ('tK, 'tV) map = function
  | v_Key, v_Alter, v_Map -> (
...
let rec singleton'2 : 'tK * 'tV -> ('tK, 'tV) map = function
  | v_Key, v_Value -> insert'3 (v_Key, v_Value, empty'0 ())

let rec union'2 : ('tK, 'tV) map * ('tK, 'tV) map -> ('tK, 'tV) map = function
  | v_Map1, v_Map2 -> Map_ffi.union'2 (v_Map1, v_Map2)

let rec fold'3 : ('tK * 'tV * 'tR -> 'tR) * 'tR * ('tK, 'tV) map -> 'tR =
  function
  | v_F, v_Acc, v_Map -> Map_ffi.fold'3 (v_F, v_Acc, v_Map)

let rec map'2 : ('tK * 'tA -> 'tB) * ('tK, 'tA) map -> ('tK, 'tB) map = function
  | v_F, v_Map -> Map_ffi.map'2 (v_F, v_Map)

let rec filter'2 : ('tK * 'tV -> bool) * ('tK, 'tV) map -> ('tK, 'tV) map =
  function
  | v_F, v_Map -> Map_ffi.filter'2 (v_F, v_Map)

let rec intersect'2 : ('tK, 'tV) map * ('tK, 'tV) map -> ('tK, 'tV) map =
  function
  | v_Map1, v_Map2 ->
      filter'2 ((function v_K, _ -> member'2 (v_K, v_Map2)), v_Map1)

let rec diff'2 : ('tK, 'tV) map * ('tK, 'tV) map -> ('tK, 'tV) map = function
  | v_Map1, v_Map2 ->
...
let rec keys'1 : ('tK, 't_V) map -> 'tK list = function
  | v_Map -> Map_ffi.keys'1 v_Map

let rec values'1 : ('t_K, 'tV) map -> 'tV list = function
  | v_Map -> Map_ffi.values'1 v_Map

let rec to_list'1 : ('tK, 'tV) map -> ('tK * 'tV) list = function
  | v_Map -> Map_ffi.to_list'1 v_Map

let rec from_list'1 : ('tK * 'tV) list -> ('tK, 'tV) map = function
