module List_priv : sig
  ...
  let rec cons'2 : 'tA * 'tA list -> 'tA list = function
    | v_H, v_T -> v_H :: v_T
  ...
  let rec maybe_cons'3 : ('tA -> 'tB Maybe.maybe'1) * 'tA * 'tB list -> 'tB list
    ...
    | v_F, v_Mx, v_Xs -> (
        match v_F v_Mx with
        | Maybe.Maybe'Just v_X -> cons'2 (v_X, v_Xs)
        | Maybe.Maybe'Nothing -> v_Xs )

  let rec maybe_cons'1 :
      ('tA -> 'tB Maybe.maybe'1) -> 'tA * 'tB list -> 'tB list = function
  ...
  let rec maximum'1 : 'tA list -> 'tA Maybe.maybe'1 = function
    | [] -> Maybe.Maybe'Nothing
    | v_H :: v_T -> Maybe.Maybe'Just (foldl'3 (Basics.max'2, v_H, v_T))

  let rec minimum'1 : 'tA list -> 'tA Maybe.maybe'1 = function
    | [] -> Maybe.Maybe'Nothing
    | v_H :: v_T -> Maybe.Maybe'Just (foldl'3 (Basics.min'2, v_H, v_T))
  ...
  let rec partition'2 : ('tA -> bool) * 'tA list -> 'tA list * 'tA list =
    function
    | v_Pred, v_List ->
        let v_Step = function
