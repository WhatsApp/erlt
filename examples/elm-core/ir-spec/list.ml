...
let rec cons'2 : 'A * 'A list -> 'A list = function v_H, v_T -> v_H :: v_T

let rec map'2 : ('A -> 'B) * 'A list -> 'B list = function
  | v_F, v_H :: v_T -> v_F v_H :: map'2 (v_F, v_T)
  | v_F, [] -> []

let rec foldl'3 : ('A * 'B -> 'B) * 'B * 'A list -> 'B = function
  | v_F, v_Acc, v_H :: v_T -> foldl'3 (v_F, v_F (v_H, v_Acc), v_T)
  | v_F, v_Acc, [] -> v_Acc

let rec foldr'3 : ('A * 'B -> 'B) * 'B * 'A list -> 'B = function
  | v_F, v_Acc, v_H :: v_T -> v_F (v_H, foldr'3 (v_F, v_Acc, v_T))
  | v_F, v_Acc, [] -> v_Acc

let rec filter'2 : ('A -> bool) * 'A list -> 'A list = function
  | v_F, v_List ->
      foldr'3
        ( (function
          | v_X, v_Xs -> (
...
let rec maybe_cons'3 : ('A -> 'B Maybe.maybe) * 'A * 'B list -> 'B list =
  function
  | v_F, v_Mx, v_Xs -> (
      match v_F v_Mx with
...
let rec append'2 : 'A list * 'A list -> 'A list = function
  | v_Xs, v_Ys -> v_Xs @ v_Ys

let rec concat'1 : 'A list list -> 'A list = function
  | v_Lists -> foldr'3 (append'2, [], v_Lists)

let rec concat_map'2 : ('A -> 'B list) * 'A list -> 'B list = function
  | v_F, v_Xs -> concat'1 (map'2 (v_F, v_Xs))

let rec intersperse'2 : 'A * 'A list -> 'A list = function
  | v_Sep, v_H :: v_T ->
...
let rec map2'3 : ('A * 'B -> 'Res) * 'A list * 'B list -> 'Res list = function
  | v_F, v_H1 :: v_T1, v_H2 :: v_T2 ->
      v_F (v_H1, v_H2) :: map2'3 (v_F, v_T1, v_T2)
  | _, _, _ -> []

let rec indexed_map'2 : (int * 'A -> 'B) * 'A list -> 'B list = function
  | v_F, v_Xs -> map2'3 (v_F, range'2 (0, length'1 v_Xs - 1), v_Xs)

let rec is_empty'1 : _ list -> bool = function _ :: _ -> false | [] -> true

let rec head'1 : 'A list -> 'A Maybe.maybe = function
  | v_H :: _ -> Maybe.Just v_H
  | [] -> Maybe.Nothing

let rec tail'1 : 'A list -> 'A list Maybe.maybe = function
  | _ :: v_T -> Maybe.Just v_T
  | [] -> Maybe.Nothing

let rec take'2 : int * 'A list -> 'A list = function
  | v_N, v_L when v_N <= 0 -> []
  | v_N, [] -> []
  | v_N, v_H :: v_T -> v_H :: take'2 (v_N - 1, v_T)

let rec drop'2 : int * 'A list -> 'A list = function
  | v_N, v_L when v_N <= 0 -> v_L
  | v_N, [] -> []
  | v_N, v_H :: v_T -> drop'2 (v_N - 1, v_T)

let rec partition'2 : ('A -> bool) * 'A list -> 'A list * 'A list = function
  | v_Pred, v_List ->
      let v_Step = function
...
let rec unzip'1 : ('A * 'B) list -> 'A list * 'B list = function
  | v_Pairs ->
      let v_Step = function
