...
pattern({enum, Line, Name, Variant, Fields}, Vt, Old, Bvt, St0) ->
    Result = check_enum(Line, Name, Variant, St0, fun(ResolvedName, Defs, St) ->
        pattern_fields(Fields, Line, ResolvedName, Defs, Vt, Old, Bvt, St)
    end),
...
check_fields(OuterLine, Fields, Name, Defs, Vt0, St0, CheckFun) ->
    Check = fun(Value, Vta, St) ->
        {Vtb, St1} = CheckFun(Value, Vt0, St),
        {vtmerge_pat(Vta, Vtb), St1}
    end,
    check_fields_common(Check, [], St0, Fields, Name, OuterLine, Defs).

pattern_fields(Fields, OuterLine, Name, Defs, Vt0, Old, Bvt, St0) ->
    Check = fun(Value, {Vta, Bvt1}, St) ->
        {Vtb, Bvt2, St1} = pattern(Value, Vt0, Old, Bvt, St),
        {{vtmerge_pat(Vta, Vtb), vtmerge_pat(Bvt1, Bvt2)}, St1}
    end,
    {_Seen, {Uvt, Bvt1}, St1} =
        check_fields_common(Check, {[], []}, St0, Fields, Name, OuterLine, Defs),
    {Uvt, Bvt1, St1}.
