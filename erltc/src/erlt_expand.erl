...
pattern({anon_struct, Line0, Pfs}, St0) ->
    {Pfs1, St1} = pattern_list(Pfs, St0),
...
pattern({struct_index, Line, Name0, Field0}, St0) ->
    {Name1, St1} = pattern(Name0, St0),
    {Field1, St2} = pattern(Field0, St1),
    {{struct_index, Line, Name1, Field1}, St2};
...
expr({record, Line0, Name, Is}, St) ->
    ...
    expr(
        {tuple, Line, [{atom, Line0, Name} | record_inits(record_fields(Name, St), Is)]},
        St
    );
...
expr({anon_struct, Line0, Pfs}, St0) ->
    ...
    {{anon_struct, Line0, Pfs1}, St1};
expr({anon_struct_update, Line0, Expr0, Pfs}, St0) ->
    {Expr1, St1} = expr(Expr0, St0),
    {Pfs1, St2} = expr_list(Pfs, St1),
    {{anon_struct_update, Line0, Expr1, Pfs1}, St2};
expr({anon_struct_field, Line0, Expr0, Field0}, St0) ->
    {Expr1, St1} = expr(Expr0, St0),
    {Field1, St2} = expr(Field0, St1),
    {{anon_struct_field, Line0, Expr1, Field1}, St2};
...
expr({struct_index, Line, Name0, Field0}, St0) ->
    {Name1, St1} = expr(Name0, St0),
    {Field1, St2} = expr(Field0, St1),
    {{struct_index, Line, Name1, Field1}, St2};
...
expr(
    {call, Line, {remote, _, {atom, _, erlang}, {atom, _, is_record}}, [A, {atom, _, Name}]},
    St
) ->
    record_test(Line, A, Name, St);
expr(
...
expr({op, Line, Op, A0}, St0) ->
    {A, St1} = expr(A0, St0),
    {{op, Line, Op, A}, St1};
expr({op, Line, Op, L0, R0}, St0) when Op =:= 'and'; Op =:= 'or' ->
    {L, St1} = bool_operand(L0, St0),
    {R, St2} = bool_operand(R0, St1),
    {{op, Line, Op, L, R}, St2};
expr({op, Line, Op, L0, R0}, St0) when Op =:= 'andalso'; Op =:= 'orelse' ->
    {L, St1} = bool_operand(L0, St0),
    {R, St2} = bool_operand(R0, St1),
...
strict_record_tests([strict_record_tests | _]) ->
    true;
strict_record_tests([no_strict_record_tests | _]) ->
    false;
strict_record_tests([_ | Os]) ->
    strict_record_tests(Os);
%Default.
strict_record_tests([]) ->
    true.

strict_record_updates([strict_record_updates | _]) ->
    true;
strict_record_updates([no_strict_record_updates | _]) ->
    false;
strict_record_updates([_ | Os]) ->
...
opt_remove_2(
    {call, Line, {atom, _, is_record}, [{var, _, V}, {atom, _, Tag}, {integer, _, Sz}]} = A,
    Rs
) ->
    case orddict:find(V, Rs) of
        {ok, {remove, Tag, Sz}} ->
