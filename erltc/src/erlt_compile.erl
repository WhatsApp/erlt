...
run_sterlang(St) ->
    ...
    SterlangPath = filename:join([
        priv_dir(),
        "sterlang-" ++ PlatformName
    ]),
    UseSterlangJar = (os:getenv("USE_STERLANG_JAR") =/= false),
    SterlangJarPath = filename:join([
        priv_dir(),
        "sterlang.jar"
    ]),
    Res =
        if
            node() =:= ?NODE_NAME_FOR_ERLT_DEVELOPMENT ->
            ...
            UseSterlangJar ->
                case filelib:is_regular(SterlangJarPath) of
                    true ->
                    ...
                    false ->
                        Msg =
                            "~nCould not find the `sterlang.jar' type checker at " ++ SterlangJarPath ++
                            "~nPlease build `sterlang.jar` by running make in <erlt repo root>/sterlang.~n~n",
                        io:format(standard_error, Msg, []),
                        error
                end;
            true ->
                % real users should only ever hit this code path
                case filelib:is_regular(SterlangPath) of
                    true ->
                        file:change_mode(SterlangPath, 8#777),
                        CheckCmd = io_lib:format("~s ~s", [SterlangPath, EtfFile]),
                        {native, spawn_sterlang(CheckCmd)};
                    false ->
                        Msg =
                            "~nCould not find the `sterlang' type checker binary for platform " ++ PlatformName ++ " at " ++ SterlangPath ++
                            "~nPlease build `sterlang.jar` by running make in <erlt repo root>/sterlang."
                            "~nThen call erlc with USE_STERLANG_JAR=true evironment variable.~n~n",
                        io:format(standard_error, Msg, []),
                        error
                end
        end,
