...
expand_opts(Opts0) ->
    ...
    Opts =
        case
            {proplists:get_value(debug_info_key, Opts0),
                proplists:get_value(encrypt_debug_info, Opts0),
                proplists:get_value(debug_info, Opts0)}
        of
    ...
    Opts1 =
        case proplists:is_defined(makedep_side_effect, Opts) of
...
format_error({unimplemented_instruction, Instruction}) ->
    io_lib:fwrite(
        "native-code compilation failed because of an "
        "unimplemented instruction (~s).",
        [Instruction]
    );
format_error({native, E}) ->
    io_lib:fwrite(
        "native-code compilation failed with reason: ~tP.",
        [E, 25]
    );
format_error({native_crash, E, Stk}) ->
    io_lib:fwrite(
        "native-code compilation crashed with reason: ~tP.\n~tP\n",
        [E, 25, Stk, 25]
    );
format_error({open, E}) ->
...
run_sub_passes_1([{Name, Run} | Ps], Runner, St0) when is_atom(Name), is_function(Run, 1) ->
...
preprocess_asm_forms(Forms) ->
    ...
    {R1#asm_module.module,
        {R1#asm_module.module, R1#asm_module.exports, R1#asm_module.attributes,
            reverse(R1#asm_module.functions), R1#asm_module.labels}}.
...
makedep(Code0, #compile{ifile = Ifile, ofile = Ofile, options = Opts} = St) ->
    ...
    Target1 =
        case proplists:get_value(makedep_quote_target, Opts) of
            true ->
    ...
    %%   common.hrl:
    Makefile =
        case proplists:get_value(makedep_phony, Opts) of
...
makedep_output(Code, #compile{options = Opts, ofile = Ofile} = St) ->
    ...
    %% the output file.
    Output0 =
        case proplists:get_value(makedep_output, Opts) of
            undefined ->
...
encrypt({des3_cbc = Type, Key, IVec, BlockSize}, Bin0) ->
    Bin1 =
        case byte_size(Bin0) rem BlockSize of
            0 -> Bin0;
...
native_compile_1(Code, St) ->
    ...
    of
        ...
        exit:{unimplemented_instruction, _} = Unimplemented ->
            Ws = [{St#compile.ifile, [{none, ?MODULE, Unimplemented}]}],
            {ok, Code, St#compile{warnings = St#compile.warnings ++ Ws}};
        Class:R:Stack ->
            case IgnoreErrors of
                true ->
                    Ws = [{St#compile.ifile, [{none, ?MODULE, {native_crash, R, Stack}}]}],
                    {ok, Code, St#compile{warnings = St#compile.warnings ++ Ws}};
                false ->
                ...
                {error, RenameError} ->
                    ...
                    Es =
                        case file:delete(Tfile) of
                            ...
                            {error, DeleteError} ->
                                Es0 ++
                                    [{Ofile, [{none, ?MODULE, {delete_temp, Tfile, DeleteError}}]}]
                        end,
                    {error, St#compile{errors = St#compile.errors ++ Es}}
            end;
        {error, Error} ->
...
-spec iofile(atom() | file:filename_all()) -> {file:name_all(), file:name_all()}.
...
make_erl_options(Opts) ->
    ...
    Options ++
        [report_errors, {cwd, Cwd}, {outdir, Outdir} | [{i, Dir} || Dir <- Includes]] ++ Specific.
